...
const connection = new signalR.HubConnectionBuilder()
    ...
    .withUrl("https://ec2-18-133-250-2.eu-west-2.compute.amazonaws.com/creyatifchat")
    //.withUrl("https://localhost:7288/creyatifchat")
    .withAutomaticReconnect([0, 100, 800, 1000, 1200, 1400, 1600, 1800, 2000, 2100, 2900])
...
connection.on("BroadcastMessage", (message) => {
    ...
    div_card.classList.add('card', 'announcement-card');
    ...
    small.append(obj.time_sent);

    const div_card_body = document.createElement("div");
    div_card_body.classList.add('card-body');

    const p = document.createElement("p");
    p.classList.add('card-text', 'font-12');
    p.style.marginTop = '-30px';
    p.append(obj.announcement);

    div_card_header.appendChild(small);
    div_card_body.appendChild(p);

    div_card.append(div_card_header, div_card_body);

    document.getElementById("announcements_div").appendChild(div_card);
...
connection.on("GroupShake", (message) => {
    document.getElementById("groupCount").innerText = message;
    console.log(message);
